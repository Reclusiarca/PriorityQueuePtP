...
RESULTADO simulacion (uint32_t nCsma, DataRate tasa_izquierda, DataRate tasa_derecha, DataRate tasa_cuello, double t_retardo_cuello, double t_retardo_izquierda, double t_retardo_derecha, int tamCola, uint32_t equiposVoIP, uint32_t equiposFTP, uint32_t equiposHTTP ) 
  ...
  double minHablando=2.0;
  double maxHablando=5.0;
  double minSilencio=0.5;
  double maxSilencio=2.5;

  double minOnHTTP=4.4;
  double maxOnHTTP=6.2;
  ...
  PacketSinkHelper sinkHTTP ("ns3::TcpSocketFactory", Address (InetSocketAddress (Ipv4Address::GetAny(), PORT_HTTP)));
...
{
// El programa sólo saca 1 gráfica, aunque hay dos disponibles. Para elegir qué grafica
// sacar hay que modificar la variable gráfica.
  ...
  int grafica = 1; //0 grafica A -- 1 grafica B
  //Para gráfico A
  ...
  if (equiposHTTP+equiposVoIP+equiposFTP == nCsma-1){
	...
	//calculo de los datoa para añadir a la gráfica
      ...
      plot[1].GenerateOutput (plotFile1);
      plotFile1 << "pause -1" << std::endl;
      plotFile1.close ();
    }
